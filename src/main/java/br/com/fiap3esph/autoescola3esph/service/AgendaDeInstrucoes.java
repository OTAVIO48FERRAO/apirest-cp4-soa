...
public class AgendaDeInstrucoes {
    ...
    @Transactional
    public DetalhamentoAgendamento cancelar(Long id, DadosCancelamento dados) {
        ...
        if (instrucao.isCancelada()) {
            throw new ValidacaoException("Esta instrução já foi cancelada anteriormente!");
        }

        long antecedenciaHoras = Duration.between(LocalDateTime.now(), instrucao.getDataHora()).toHours();
        if (antecedenciaHoras < 24) {
            throw new ValidacaoException("Uma instrução somente pode ser cancelada com antecedência mínima de 24 horas!");
        }
